...
import (
	...
	"github.com/MichaelSp/prom-migrator/pkg/log"
	plan "github.com/MichaelSp/prom-migrator/pkg/planner"
...
type config struct {
	...
	concurrentPull       int
	concurrentPush       int
	readerClientConfig   remote.ClientConfig
	maxReadDuration      time.Duration // Max range of slab in terms of time.
	writerClientConfig   remote.ClientConfig
...
func main() {
	...
	planConfig := &plan.Config{
		...
		ReadClientConfig:     conf.readerClientConfig,
		WriteClientConfig:    conf.writerClientConfig,
	}
...
func parseFlags(conf *config, args []string) {
	...
	flag.StringVar(&readUrl, "reader-url", "", "URL address for the storage where the data is to be read from.")

	flag.DurationVar((*time.Duration)(&conf.readerClientConfig.Timeout), "reader-timeout", defaultTimeout, "Timeout for fetching data from read storage. "+
		"This timeout is also used to fetch the progress metric.")
	flag.BoolVar(&conf.readerClientConfig.RetryOnRateLimit, "retry-on-rate-limit", defaultRetryOnRateLimit, "Retry on rate-limit is the time to wait before retrying to fetch data from the read storage")
	...
	if conf.readerClientConfig.Headers == nil {
		conf.readerClientConfig.Headers = make(map[string]string)
	}
	flag.Var(&utils.HeadersFlag{Headers: conf.readerClientConfig.Headers}, "reader-http-header", "HTTP header to send with all the reader requests. It uses the format `key:value`, for example `-reader-http-header=\"X-Scope-OrgID:42\"`. Can be set multiple times to define several headers or multiple values for the same header.")

	flag.StringVar(&writeUrl, "writer-url", "", "URL address for the storage where the data migration is to be written.")
	flag.DurationVar((*time.Duration)(&conf.writerClientConfig.Timeout), "writer-timeout", defaultTimeout, "Timeout for pushing data to write storage.")

	if conf.writerClientConfig.Headers == nil {
		conf.writerClientConfig.Headers = make(map[string]string)
	}
	flag.Var(&utils.HeadersFlag{Headers: conf.writerClientConfig.Headers}, "writer-http-header", "HTTP header to send with all the writer requests. It uses the format `key:value`, for example `-writer-http-header=\"X-Scope-OrgID:42\"`. Can be set multiple times to define several headers or multiple values for the same header.")
	...
	conf.readerClientConfig = getDefaultClientConfig(readUrl)
	conf.writerClientConfig = getDefaultClientConfig(writeUrl)
}

func getDefaultClientConfig(urlString string) remote.ClientConfig {
...
func validateConf(conf *config) error {
	...
	case strings.TrimSpace(conf.readerClientConfig.URL.String()) == "" && strings.TrimSpace(conf.writerClientConfig.URL.String()) == "":
