...
func (s *Slab) Fetch(ctx context.Context, client remote.ReadClient, mint, maxt int64, matchers []*labels.Matcher) (err error) {
	var (
	...
	for i := 0; i < totalRequests; i++ {
		...
		i := i
		go func() {
			result, err := client.Read(cctx, readRequest)
			if err == nil {
				responseChan <- &Resp{
					ID:     i,
					Result: result,
	...
	var (
		...
		pendingResponses = make([]*prompb.QueryResult, totalRequests)
	)
	for i := 0; i < totalRequests; i++ {
		resp := <-responseChan
		switch response := resp.(type) {
		case *Resp:
			bytesUncompressed += response.Result.Size()
			...
			pendingResponses[response.ID] = response.Result
		...
		default:
			panic(any("invalid response type"))
		}
	}
	close(responseChan)
	...
	if totalRequests > 1 {
		...
		s.timeseries = pendingResponses[0].Timeseries
	}
...
func (s *Slab) mergeSubSlabsToSlab(subSlabs []*prompb.QueryResult) ([]*prompb.TimeSeries, error) {
...
func (s *Slab) Done() error {
	...
	s.SetDescription(
		fmt.Sprintf("Memory footprint: %.2f MB.", float64(s.numBytesUncompressed)/float64(utils.Megabyte)),
		1,
	)
	s.done = true
