...
type Config struct {
	...
	ProgressClientConfig remote.ClientConfig
	ReadClientConfig     remote.ClientConfig
	WriteClientConfig    remote.ClientConfig
...
func Init(config *Config) (*Plan, bool, error) {
	...
	if config.ProgressEnabled {
		if config.ProgressClientConfig.URL == nil {
...
func (c *Config) fetchLastPushedMaxt() (lastPushedMaxt int64, found bool, err error) {
	...
	readClient, err := remote.NewReadClient("reader-last-maxt-pushed", &c.ReadClientConfig)
	if err != nil {
